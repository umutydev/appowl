...
import 'package:firebase_auth/firebase_auth.dart'; // Kullanıcı kontrolü için
...
class FavoritesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // KULLANICI GİRİŞ YAPMAMIŞSA DİREKT UYARI VER (DÖNMEZ)
      ...
      body: user == null
          ...
          : StreamBuilder<List<Place>>(
              ...
              builder: (context, snapshot) {
                // EĞER FİREBASE HATA FIRLATIRSA DÖNMESİN, HATAYI YAZSIN
                if (snapshot.hasError) {
                  return Center(
                    child: Text(
                      "Veriler çekilemedi: ${snapshot.error}",
                      style: const TextStyle(color: Colors.redAccent),
                    ),
                  );
                }

                // SADECE VERİ YÜKLENİYORKEN (İlk saniye) DÖNSÜN
                ...
                // LİSTE BOŞSA NET BİR ŞEKİLDE UYARI VERSİN
                if (places.isEmpty) {
                  return const Center(
                    child: Column(
